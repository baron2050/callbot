...
/*** DOCUMENTATION
	...
	<agi name="asyncstream file" language="en_US">
		<synopsis>
			Sends audio file on channel asynchronously.
		</synopsis>
		<syntax>
			<parameter name="filename" required="true">
				<para>File name to play. The file extension must not be
		...
		<description>
			<para>send audio file on channel, and return immediately</para>
		</description>
	</agi>
	<agi name="stopstream" language="en_US">
		<synopsis>
			Stop the strea playing on the channel.
		</synopsis>
		<description>
			<para>send audio file on channel, and return immediately</para>
		</description>
    ...
    <agi name="getaudiostream" language="en_US">
        <synopsis>
            Get audio stream
        </synopsis>
        <syntax>
        ...
        <description>
            <para>get realtime audio stream to specified host port</para>
        </description>
    </agi>
...
static int handle_asyncstreamfile(struct ast_channel *chan, AGI *agi, int argc, const char * const argv[])
{
    int res;
...
static int handle_stopstream(struct ast_channel *chan, AGI *agi, int argc, const char * const argv[])
{
	if (argc < 1)
		return RESULT_SHOWUSAGE;
    ast_stopstream(chan);
...
{
    int hangup = ast_check_hangup(chan);
    if (hangup) {
        ast_agi_send(agi->fd, chan, "511 HANGUP\n");
    } else {
        ast_agi_send(agi->fd, chan, "200 result=0\n");
    }
	return RESULT_SUCCESS;
...
static int handle_getaudiostream(struct ast_channel *chan, AGI *agi, int argc, const char * const argv[])
{
    ast_log(LOG_NOTICE, "GAS: getaudiostream argc=%d, host=%s, port=%s\n", argc, argv[1], argv[2]);
    ...
    if (!ast_gethostbyname(host, &hp)) {
        ast_log(LOG_ERROR, "GAS: unable to gethostbyname: host=%s, error=%s\n", host, strerror(errno));
        ast_agi_send(agi->fd, chan, "200 result=-1\n");
        return RESULT_FAILURE;
    }
    ...
    if (sock < 0) {
        ast_log(LOG_ERROR, "GAS: unable to create socket: %s\n", strerror(errno));
        ast_agi_send(agi->fd, chan, "200 result=-1\n");
        return RESULT_FAILURE;
    }
    ...
    if (flags < 0) {
        ast_log(LOG_ERROR, "GAS: unable to GETFL: %s\n", strerror(errno));
        close(sock);
        ast_agi_send(agi->fd, chan, "200 result=-1\n");
        return RESULT_FAILURE;
    }
    ...
    if (fcntl(sock, F_SETFL, flags|O_NONBLOCK) < 0) {
        ast_log(LOG_ERROR, "GAS: unable to set nonblock mode: %s\n", strerror(errno));
        close(sock);
        ast_agi_send(agi->fd, chan, "200 result=-1\n");
        return RESULT_FAILURE;
    }
    ...
    memcpy(&remote_address.sin_addr.s_addr, hp.hp.h_addr, hp.hp.h_length);

    if (connect(sock, (struct sockaddr*)&remote_address, sizeof(remote_address)) && (errno != EINPROGRESS))  {
        ast_log(LOG_ERROR, "GAS: unable to connect to stream host '%s:%d': %s\n", host,port, strerror(errno));
        close(sock);
        ast_agi_send(agi->fd, chan, "200 result=-1\n");
        return RESULT_FAILURE;
    }
    ...
    while ((res = ast_poll(pfds, 1, MAX_AGI_CONNECT)) != 1)  {
        if (errno != EINTR) {
            if (!res) {
                ast_log(LOG_WARNING, "GAS: connection to '%s:%d' timed out after MAX_AGI_CONNECT (%d) milliseconds.\n",
                        host, port, MAX_AGI_CONNECT);
            } else {
    ...
    const char* callerId = "unknown";
    if (chan->caller.id.number.valid) {
        callerId = chan->caller.id.number.str;
    }
    struct iovec vec[2];

    ast_log(LOG_NOTICE, "GAS: callerId=%s\n", callerId);

    uint16_t caller_len = htons((uint16_t)strlen(callerId));
    vec[0].iov_base = &caller_len;
    vec[0].iov_len = sizeof(caller_len);
    vec[1].iov_base = callerId;
    vec[1].iov_len = strlen(callerId);
    res = writev(sock, vec, 2);
    if (res < 0) {
        ast_log(LOG_ERROR, "GAS: send stream header failed to '%s:%d': %s\n", host, port, strerror(errno));
        ast_agi_send(agi->fd, chan, "200 result=-1\n");
        close(sock);
        return  RESULT_FAILURE;
    }
    ...
    ast_agi_send(agi->fd, chan, "200 result=0\n");
    ast_log(LOG_NOTICE, "GAS: connected to '%s:%d', sockfd=%d\n", host, port, sock);
...
static struct agi_command commands[] = {
	...
	{ { "asyncstream", "file", NULL }, handle_asyncstreamfile, NULL, NULL, 0 },
	{ { "stopstream", NULL }, handle_stopstream, NULL, NULL, 0 },
    ...
    { { "getaudiostream", NULL }, handle_getaudiostream, NULL, NULL, 0 },
};
...
static int agi_exec_full(struct ast_channel *chan, const char *data, int enhanced, int dead)
{
	...
	if (res == AGI_RESULT_SUCCESS || res == AGI_RESULT_SUCCESS_FAST) {
		...
		if (fds[1] != fds[0])
			close(fds[1]);
		if (agi.audio > -1) {
            ast_log(LOG_NOTICE, "GAS: disconnected sockfd=%d\n", agi.audio);
			close(agi.audio);
        }
	}
