...
public interface AgiChannel
{
    ...
    /**
     * Sends a command to asterisk and returns the corresponding reply. The
     * reply is also available through {@link #getLastReply()}.
     *
     ...
     * @since 0.2
     */
    String getData(String file, long timeout, int maxDigits) throws AgiException;
     ...
     * @since 0.2
     */
    char getOption(String file, String escapeDigits, long timeout) throws AgiException;
     ...
     * @throws AgiException
     */
    void confbridge(String room, String profile) throws AgiException;
     ...
     * @throws AgiException
     */
    void meetme(String room, String options) throws AgiException;
	...
	/**
	 *  Plays the given file asynchronously.
	 * @param file
	 * @return 0 on success, -1 on error.
	 * @throws AgiException
	 */
	int asyncStreamFile(String file) throws AgiException;

	/**
	 * Stop the stream playing list on channel;
	 * @throws AgiException
	 */
	void stopStream() throws AgiException;
	...
	/**
	 * Get the audio stream being sent to the specified audio stream server
	 * @param host the host of audio stream server
	 * @param port the port of audio stream server
	 * @return 0, if success; others, failed.
	 * @throws AgiException
	 */
	int getAudioStream(String host, int port) throws AgiException;
    ...
    /**
     * Returns the value of the current channel or global variable. <br>
     * Supports functions and builtin variables. To retrieve the caller id you
     * can use <code>getVariable("CALLERID(name)");</code> <br>
     * Does not support expression parsing, use {@link #getFullVariable(String)}
     * in those cases.
     *
     * @param name the name of the variable (or function call) to retrieve.
     * @return the value of the given variable or <code>null</code> if not set.
     * @since 0.2
     */
    ...
    /**
     * Evaluates a channel expression for the current channel. To extract the
     * caller id use <code>getFullVariable("${CALLERID(name)}");</code>.
     * <p>
     ...
     * <code>getFullVariable("${CALLERID(name)}", "SIP/john-0085d860");</code>.
     * <p>
     * Available since Asterisk 1.2.
     *
     ...
     * @since 0.2
     */
    char sayDateTime(long time, String escapeDigits, String format, String timezone) throws AgiException;
     ...
     * @since 0.3
     */
    char recordFile(String file, String format, String escapeDigits, int timeout) throws AgiException;
     ...
     * @since 0.3
     */
    char recordFile(String file, String format, String escapeDigits, int timeout, int offset, boolean beep, int maxSilence)
            throws AgiException;
    ...
    /**
     * Creates a speech object that uses the given speech engine. The speech
     * object is used by the other speech methods and must be created before
     * they are called.
     *
    ...
    /**
     * Loads the specified grammar. The grammer is then available for calls to
     * {@link #speechActivateGrammar(String)} under the given name. Eplicitly
     * loading a grammer is only required if the grammer has not been defined in
     * the speech engine configuration, e.g. the <code>[grammars]</code> section
